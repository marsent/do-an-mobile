...
import { errorColor, mainBlack, mainBlue, mainGray, mainWhite } from '../style/color'
...
const TextInput = ({
    ...
    return (
        <View style={{ paddingTop: 11 }}>
            {
                label && (hasFocus || !value == false) && <Animatable.View
                    animation={fadeIn}
                    duration={500}
                    style={{ position: 'absolute', top: 0, marginLeft: !leftIcon ? 15 : 50, backgroundColor: mainWhite, borderColor: mainWhite, zIndex: 100 }}
                >
                    <Text
                        size={14}
                        color={borderColor}
            ...
            <View style={[{ flexDirection: 'row', alignItems: 'center', borderRadius: 10, borderColor: hasFocus ? focusColor : outlineColor, borderWidth: outLine ? 1 : hasFocus ? 1 : 0, zIndex: 0, elevation: shadow ? shadow : 0 }, style, !errorMessage ? null : styles.borderErr,]}>
                ...
                <TextInputBase
                    ...
                    multiline={multiline}
                    style={[{ flex: 1, marginLeft: 10, fontFamily: 'Inter', fontSize: 14, color: mainBlack }]}
